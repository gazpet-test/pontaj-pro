...
// ── 22.09.2026: logica e aici, ca sa fie rulata si de workerul de pe NAS (worker/ofertare/main.ts) —
// fara limita de 150 s a gateway-ului. index.ts (edge function) = autorizare + invelis HTTP peste extrageCerinte().
export type Rezultat = Record<string, any>

export async function extrageCerinte(supabase: any, body: any): Promise<Rezultat> {

  const fail = (msg: string): Rezultat => ({ error: msg })

  try {
    const { licitatie_id, sectiune, reset, doc_id, bucata, bucata_max, model } = body || {}
    ...
    if (modCorpus) {
      ...
      if (t.length < 300) return { ok: true, doc_id: doc.id, cerinte: 0, skip: 'text prea scurt', continua: false }
      text = t
      srcDocId = doc.id
      ...
      if (sectiune === 'II') {
        if (iII < 0) return { ok: true, sectiune, cerinte: 0, skip: 'ancora II lipsa', continua: false }
        text = tot.slice(iII, iIII > iII ? iIII : undefined)
      } else if (sectiune === 'III') {
        if (iIII < 0) return { ok: true, sectiune, cerinte: 0, skip: 'ancora III lipsa', continua: false }
        text = tot.slice(iIII, iIV > iIII ? iIV : (iVfin > iIII ? iVfin : undefined))
      } else if (sectiune === 'IV') {
        if (iIV < 0) return { ok: true, sectiune, cerinte: 0, skip: 'ancora IV lipsa', continua: false }
    ...
    if (nrBucata >= toate.length) return { ok: true, cerinte: 0, skip: 'nu mai sunt bucati', continua: false }
