...
//
// Rulează din cron (meteo_sync_orar, la fiecare oră) cu antet x-meteo-secret = Vault METEO_CRON_SECRET,
  ...
  for (const s of tinte) {
    try {
      const [cond, fc, al] = await Promise.all([
        ...
        get(`forecasts/${loc(s)}?filter=day&limit=4&fields=periods.dateTimeISO,periods.maxTempC,periods.minTempC,periods.weather,periods.icon,periods.precipMM,periods.pop,periods.windSpeedMaxKPH,periods.windGustKPH`),
        get(`alerts/${loc(s)}?limit=5&fields=details.name,details.type,timestamps.beginsISO,timestamps.expiresISO`).catch(() => []),
      ]);
      ...
      const prognoza = ((Array.isArray(fc) ? fc[0] : fc)?.periods || []).map((d: any) => ({ data: (d.dateTimeISO || '').slice(0, 10), max: d.maxTempC, min: d.minTempC, vreme: d.weather, icon: d.icon, precip_mm: d.precipMM, prob: d.pop, vant_kph: d.windSpeedMaxKPH, rafale_kph: d.windGustKPH }));
      const alerte = (Array.isArray(al) ? al : []).map((a: any) => ({ titlu: a.details?.name, tip: a.details?.type, de_la: a.timestamps?.beginsISO, pana_la: a.timestamps?.expiresISO }));
      await db.from('meteo_cache').upsert({ site_id: s.id, curent, prognoza, alerte, actualizat_la: new Date().toISOString(), sursa: 'xweather' }, { onConflict: 'site_id' });
      rez.push({ site: s.id, ok: true, temp: p.tempC, vreme: p.weather, alerte: alerte.length });
    } catch (e) {
