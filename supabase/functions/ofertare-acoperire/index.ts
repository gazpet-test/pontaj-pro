...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: recAll, error: eRec } = await supabase.from('hr_recomandari')
      ...
      .select('id, employee_id, extern_id, rol, beneficiar, obiect_lucrare, perioada_start, perioada_end, valoare_lei, domenii, verificat, ai_confidenta, nr_document, data_document, calificativ, emp:employees(name, active), ext:hr_personal_extern(nume, activ)')
      .eq('activ', true).order('id')
