-- Aplicarea unui raspuns al autoritatii asupra registrului de cerinte. O singura tranzactie.
--
-- Primeste ID-uri de operatii, NU texte: propunerea sta in ofertare_raspuns_set.propunere, generata de
-- server. Altfel clientul ar putea trimite orice text si el ar intra direct in registru.
--
-- Aplicare PARTIALA, nu abort total: o linie a carei amprenta nu mai corespunde nu se aplica niciodata,
-- dar se intoarce in `conflicte` cu textul curent, iar restul trec. Altfel o singura editare fara
-- legatura ar arunca tot triajul omului.
--
-- ACOPERIREA SE MUTA (UPDATE cerinta_id), nu se copiaza: indexul unic partial
-- ofertare_acoperire_cerinta_unic_nevalidat permite UN singur rand nevalidat per cerinta, iar o copiere
-- „toate cu verificat_pe_scan=false" ar fi picat pe cerintele cu mai multe acoperiri (1061 are 3).
-- Inainte de mutare se scrie acoperire_snapshot pe versiunea VECHE — singurul loc din care se mai poate
-- afla ce dovezi sustineau textul vechi.
--
-- Probe rulate 12.09 pe o licitatie de unica folosinta, stearsa dupa:
--   modificare + anulare + cerinta noua + o linie cu amprenta stricata -> 3 aplicate, 1 in conflicte;
--   cerinta cu 3 acoperiri (2 verificate pe scan + 1 nevalidata) -> toate 3 mutate, verificat_pe_scan
--     pastrat, zero coliziuni pe indexul unic;
--   nr_ordine mostenit (versiunea noua ramane pe locul celei vechi, nu sare la coada);
--   reluare cu acelasi idempotency_key -> acelasi rezultat, zero scrieri noi;
--   a doua aplicare cu alt key -> 4 conflicte, zero scrieri;
--   status pierduta -> refuz; depusa fara bifa -> refuz; depusa cu bifa dar fara motiv -> refuz;
--   eroare injectata la a doua operatie -> rollback integral (prima operatie, deja scrisa, s-a anulat).
...
DECLARE
  ...
  v_nou_id bigint; v_amp text; v_amp_prop text; v_text_nou text;
...
COMMENT ON FUNCTION public.fn_ofertare_raspuns_set_aplica(bigint, text[], uuid, boolean, text, text) IS
  'Aplica asupra registrului de cerinte operatiile bifate dintr-o propunere salvata pe set. O singura tranzactie, aplicare partiala cu lista de conflicte, acoperirea se muta si se marcheaza pentru reverificare, iar fotografia ei ramane pe versiunea veche.';
REVOKE EXECUTE ON FUNCTION public.fn_ofertare_raspuns_set_aplica(bigint, text[], uuid, boolean, text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.fn_ofertare_raspuns_set_aplica(bigint, text[], uuid, boolean, text, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.fn_ofertare_raspuns_set_aplica(bigint, text[], uuid, boolean, text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.fn_ofertare_raspuns_set_aplica(bigint, text[], uuid, boolean, text, text) TO service_role;
