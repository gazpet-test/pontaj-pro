...
CREATE TABLE IF NOT EXISTS public.ofertare_cerinte_dovezi (
  ...
  CONSTRAINT ofertare_cerinte_dovezi_sha_chk CHECK (fisier_sha256 IS NULL OR fisier_sha256 ~ '^[0-9a-f]{64}$'),
  CONSTRAINT ofertare_cerinte_dovezi_locator_chk CHECK (jsonb_typeof(locator) = 'object'),
...
COMMENT ON COLUMN public.ofertare_cerinte_dovezi.locator IS 'Obiect: {"pagina": 13} | {"interval": [3,5]} | {"verificat": "document"} + opțional {"sectiune": "III.1.7"}; paginile sunt cele ale fișierului original.';
...
CREATE OR REPLACE FUNCTION public.fn_ofertare_cerinte_dovada_adauga(
  ...
  p_document_id bigint DEFAULT NULL, p_locator jsonb DEFAULT '{}'::jsonb,
  p_fisier_sha256 text DEFAULT NULL, p_corecteaza_id bigint DEFAULT NULL)
...
BEGIN
  ...
  INSERT INTO public.ofertare_cerinte_dovezi (cerinta_id, text_md5, document_id, fisier_sha256, locator, pasaj, explicatie, corecteaza_id, verificat_de)
...
END $fn$;
REVOKE ALL ON FUNCTION public.fn_ofertare_cerinte_dovada_adauga(bigint, text, text, bigint, jsonb, text, bigint) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fn_ofertare_cerinte_dovada_adauga(bigint, text, text, bigint, jsonb, text, bigint) TO authenticated, service_role;
...
SELECT d.id, d.cerinta_id, c.licitatie_id, d.document_id, doc.nume_original AS document_nume, d.fisier_sha256, d.locator,
...
COMMENT ON VIEW public.v_ofertare_cerinte_dovezi IS 'Dovezile pe cerință + pentru_textul_curent (md5 la verificare = md5 acum) + inlocuita (există o corecție ulterioară). security_invoker: RLS-ul tabelelor de bază se aplică cititorului.';
