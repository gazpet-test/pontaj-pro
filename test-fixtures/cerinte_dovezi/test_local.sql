...
SELECT CASE WHEN pentru_textul_curent AND NOT inlocuita AND document_nume = 'Fisa_de_date_TEST.pdf' AND verificat_de_nume = 'Editor Ofertare' THEN 'PASS: view — pentru textul curent, neînlocuită, document + verificator vizibile' ELSE 'FAIL T4c' END FROM public.v_ofertare_cerinte_dovezi WHERE id = 1;
...
SELECT CASE WHEN (SELECT inlocuita FROM public.v_ofertare_cerinte_dovezi WHERE id = 1) AND NOT (SELECT inlocuita FROM public.v_ofertare_cerinte_dovezi WHERE id = 2) THEN 'PASS: 1 înlocuită, 2 curentă' ELSE 'FAIL T7b' END;
...
SELECT pg_temp.asteapta_eroare($q$SELECT public.fn_ofertare_cerinte_dovada_adauga(1, 'scurt', 'explicatie ok', NULL, '{}')$q$, 'pasaj_chk');
SELECT pg_temp.asteapta_eroare($q$SELECT public.fn_ofertare_cerinte_dovada_adauga(1, 'un pasaj suficient de lung', 'explicatie ok', NULL, '{}', 'nu-e-sha')$q$, 'sha_chk');
RESET ROLE;
