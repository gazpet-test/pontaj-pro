...
while true; do
  ...
  WORKER_GIT_SHA="$SHA" REPO_BRANCH="$BRANCH" deno run --allow-net --allow-env --allow-read=/app,/deno-dir,/tmp,/packs,/seap-work --allow-write=/deno-dir,/tmp,/packs,/seap-work --allow-run=git,pdftotext,pdfinfo /app/worker/ofertare/main.ts &
  COPIL=$!
  wait "$COPIL"; COD=$?
