...
while true; do
  ...
  WORKER_GIT_SHA="$SHA" REPO_BRANCH="$BRANCH" deno run --allow-net --allow-env --allow-read=/app,/deno-dir,/tmp,/packs --allow-write=/deno-dir,/tmp,/packs --allow-run=git,pdftotext,pdfinfo,7zz /app/worker/ofertare/main.ts &
  COPIL=$!
  wait "$COPIL"; COD=$?
