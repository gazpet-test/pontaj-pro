...
| C3 reluare/2 taburi | R4 | remediat în cod (runda 3, **nedeployat**): rezervare per (doc, zonă, tăiere) înainte de AI | `concurenta.ts` `rezervaChei`/`REZERVARE_EXPIRA_MS`; `handler.ts` `planifica` + `rezervaChei`; `api/plansa-felii.js` refuz retăiere cu zone rezervate; teste `R4 rezervare:*` (55/55) | deploy edge + Vercel; test LIVE cu 2 taburi pe o planșă mică (cost) — GO Razvan |
