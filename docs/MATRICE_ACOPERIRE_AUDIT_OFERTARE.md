...
| C3 reluare/2 taburi | R4 | parțial — remediat în cod (runda 3 + corecturi verificator, **nedeployat**): rezervare per (doc, zonă, tăiere) înainte de AI; „citește” de la zero nu cooperează; plafon `pana_la`; retăiere fail-closed | `concurenta.ts` `rezervaChei`/`rezActiva`/`REZERVARE_EXPIRA_MS`; `handler.ts` `planifica` + `rezervaChei`; `api/_cas.js` `verificaRetaiere`; teste `R4 rezervare:*`, `R4 reset:*`, `R4 plafon*`; suita `ofertare-plansa-citeste` **89/89** (capul ramurii, după runda 4; după `a800d38` 78/78, după `71b74e8` 62/62), `-A supabase/functions/` 106/106, `concurenta_test.ts` 10×49/49, `test-cas-felii` 34/34, `test-detector-sigla` 23/23; agregarea tronsoanelor pe identitatea rândului (doc, pagină, tabel, Nr) — `R4_REZERVARE_ZONE_SI_COADA_NAS.md` §5. Runda 4 (verificatorul rundei 3 NU a confirmat `a800d38`): blocantul (rândul de margine numărat de două ori prin poziție + Nr; 470 +320 m tăcut) și majorele (rânduri distincte contopite B/C/C2/D; Dn doar „de verificat” cu cifra veche la transfer) sunt reparate în cod, cu 11 teste de regresie care pică pe `a800d38`; 470 rămâne 133 / 48.905 m, 130 rămâne 18 / 37.320 m; `COD_VERSIUNE` 2026-09-26.8 — **de reverificat** (§5.3, §5.5–5.7) | deploy edge + Vercel; test LIVE cu 2 taburi pe o planșă mică (cost) — GO Razvan; reziduu: fereastra tăiere+upload |
