...
| T6 rânduri repetate | R5 | remediat în cod **pe ramură** (identitatea rândului, `claude/r4-rezervare-zone` @ `3039f2a`, cod `1cc92bd`), nedeployat; majorele TOTAL-a / TOTAL-b închise în runda 12 (fail-safe: „total” nerecunoscut = ambiguu, nimic scris) | `handler.ts:596` (main); ramura: `identificaRanduri` | reverificarea adversarială a rundei 12; merge + deploy (GO); recitirea 470 (GO separat) |
...
| T12 hash/seap_cod | R6 | **ÎNCHIS tehnic** (26.09) | manifest SEAP verificat pe 878 fișiere (`verifica_manifest.ts`, #482); cron săptămânal în worker (#484) | reconcilierea înaintea înghețării pachetului final; excepția FD Jilava: dovada randării = raportul `viz_fd` (worker Terra, 25.09: text 15/15 identic, randare 72 dpi identică pixel cu pixel), nu e în repo — se atașează la dosarul pachetului; restanțele de date R6 din 25.09 rămân nereverificate |
...
| R4 reluare | PARȚIAL (perimetru acceptat) — atomicitatea aplicată (#481), nu se redeschide | rezervarea pe zonă pe ramură (`3039f2a`), nedeployată; coada independentă de browser = design + migrare propusă, neaplicată |
| R5 cantități 470 | **DESCHIS** (0 / 6 criterii închise; R5 v2 rev. 7) | protecția consumatorilor pe ramură (`a15a62a`, peste R4 `3039f2a`; condițiile Copilot 1–2 + cele 6 decizii din audit, reversibile), migrări neaplicate; deduplicarea pe ramură (`3039f2a`): majorele TOTAL-a / TOTAL-b închise în runda 12, **reverificarea adversarială a rundei 12 de făcut**; verificarea Oanei; #63; aprobarea umană; propagarea; marcajul pasului B aliniat (codul recunoaște ambele forme) |
...
| R7 Vâlcelele financiar | **DESCHIS** (structura fișei v3 acceptată; #487 corect) | deciziile nominale ale lui Razvan (lista 1–12 din fișă); verificarea umană a celor 35 de clauze (0 / 35); cl. 43 cu proveniență; utilizarea consecventă în ofertă; testul de propagare pe ramură (`80d618f`, 81/81, reverificarea rundei 2 confirmă), nemergiuit; deciziile deschise ale ramurii (veghea SEAP A / B / C, butonul „prelungirea nu mai e necesară”, polița în original + cerință necitită); criteriul 3: 90 de zile implicit scos (#487, `a4b2982`); aprobarea fișei nu reaprobă clauzele |
