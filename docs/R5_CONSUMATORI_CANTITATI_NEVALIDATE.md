...
10. **Rămâne netratat explicit** (limitări, §7): ramurile „doar de verificat” / MY-T4 / „total cu Dn” pe un rând VALIDAT scriu doar notă (cifra nu se schimbă, validarea rămâne) — altă formă de „citire care nu confirmă”; **de decis (Razvan)** dacă trec și ele pe varianta B (verificatorul rundei 5 a confirmat pe copie că e vizibil doar în notă). Istoricul nu are încă ecran (se citește din BD).
11. **Runda 5 — de confirmat:** (a) `um` se compară EXACT (orice schimbare, inclusiv „m” → „M” / „m ”, scoate rândul din „validat”) — conservator, dar aceeași definiție ca filtrele de rețea; (b) pragul de 1 m se măsoară față de valoarea de la ultima validare (după revalidare, referința e cifra nouă); (c) poarta propunerii dă WARN (nu block) când graficul înghețat nu mai corespunde cantităților de acum.
...
**Minore:**
- **`um` vs filtrul rețelei:** `um` se compară exact (orice schimbare e relevantă), aceeași definiție ca `qm` / view / v6 (`um = 'm'`). Repro A5 („m” → „M” pe un F3 validat) ⇒ invalidat, numărat în `invalidate_in_afara_retea` (700 m), H2 block. v6 numără și rândurile F3 invalidate ieșite din `qm` ⇒ fără total citat (nu mai omite tacit rândul).
...
- **Ramurile „doar de verificat” / MY-T4 / „total cu Dn”** pe un rând VALIDAT: neschimbate, rămân **decizia lui Razvan** (§6.10).
