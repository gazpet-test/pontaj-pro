...
- Ramura: `claude/cantitati-nevalidate-consumatori`, **reașezată (rebase, 26.09) peste `claude/r4-rezervare-zone` @ 8db5587** (reparațiile R4 + deduplicarea pe identitatea rândului). Nepushată, fără PR. Nimic deployat.
  - `233753a` = commit-ul R5 inițial (fost 990a6b1), cu conflictul din `treciInCantitati` rezolvat păstrând ambele semantici (§2.2);
  - `48f52d6`, `ef16e86`, `435c285` + commit-ul de documentație = **runda 4** (problemele verificatorului rundei 3, §2.1).
...
| `supabase/functions/ofertare-plansa-citeste/handler.ts` | `randDinPlansa` (runda 4: `tip_sursa` declarat decide; doar fără tip contează sursa „… citit automat din scanare”). Un astfel de rând nu mai e „dinMemoriu”. Recitirea identică dă nota „valoare din planșă, nu confirmare din memoriu”. Recitirea diferită dă „rândul are X m din citirea anterioară … verifică” și status `diferenta`. **Runda 4**: `cifraSchimbata` — și rândul `validat` (și TOTAL) trece pe `diferenta` când cifra din planșă se schimbă cu ≥ 1 m; „recitire” doar pe aceeași planșă (`dinAceeasiPlansa`), altfel ambiguu și neatins; coliziunea a două grupuri (Dn, material) pe aceeași poziție → ambiguu, fără update, ordine deterministă. `treciInCantitati` e exportat pentru test. | `cantitati_nevalidate_test.ts`: 16 teste (5 + 11 în runda 4). Contra-probă pe HEAD 8a6fbbb (runda 3): **2/5 pică comportamental**, al treilea doar pentru că exportul lipsea (corectat; „3/5” din runda 3 era greșit). Contra-probă runda 4 pe 233753a: 8/8 teste noi de comportament pică. Suita planșei: 116/116 |
...
- Două grupuri (Dn, material) care ajung la aceeași poziție (Dn63 PE + Dn63 OL, o singură poziție „Dn63”) ⇒ ambele ambigue, **niciun update**, ordine deterministă (Dn desc, apoi material). Defect preexistent (ADV5, reprodus și pe 8a6fbbb); test cu ordinea inversată.
...
- **Transferul: sursa rândului nu poartă id-ul documentului** — două planșe din documente diferite cu același nr. („Planșa 1”) se tratează ca aceeași planșă la recitire. Iar pozițiile ambigue (altă planșă, coliziune) apar doar în rezumatul JSON al transferului, nu în UI (preexistent).
