...
Cod (tichet separat, nu în acest item): `OfertareGarantie.jsx:56` recunoaște doar „NNN zile”, așa că „4 luni” duce la 90 de zile implicit; corectura = calculul din „4 luni” + termen. Avertizarea de decalare (linia 212) nu apare după starea `original` (linia 168).
...
| 3 | Parametrii greșiți corectați: fără presupunerea de 90 de zile (dar fără înlocuirea globală cu 150); cl. 43 cu proveniență; valorile și rolurile GBE distincte | **90 de zile** încă implicit în cod (`OfertareGarantie.jsx:56`); **cl. 43** încă „12 luni pentru Etapa 1”; **`garantie_participare`** încă „IBAN mascat” (329 de caractere); GBE: cl. 30 (10%, sumă garantată), 31 (termenul), 32 (0,5% rețineri) — valori și roluri distincte, corecte literal | fix de cod (calcul din „4 luni” + termen); UPDATE cl. 43 cu citat și istoric; SQL-ul de mai sus — toate prin preview → GO |
...
| 5 | Propagarea verificată (fișa licitației, cererea către broker, centralizatorul, graficul, declarațiile — aceeași versiune) + test de recalculare la modificarea termenului | **nimic propagat**: `ofertare_garantii` 0, `grafic_activitati` / `grafic_versiuni` 0, `ofertare_pt_capitole` 0, `ofertare_pt_pachet` 0, centralizatorul nu există în platformă, formularele `de_pregatit`. Recalcularea: analizată doar în cod (actualizarea păstrează numărul de zile, l. 173; avertizarea dispare după `original`), netestată pe 95 | după apply: verificarea pe cele 5 artefacte; testul de recalculare pe o licitație de probă (termen mutat ± 30 de zile), cu GO. Testul de propagare (termen mutat ⇒ cerința recalculată + semnal de reverificare; polița emisă nu e considerată prelungită) e implementat pe ramura `claude/r7-propagare-garantie` (`5e61820`, `80d618f`; vitest 81/81), nemergiuit — stare în `RESTANTE_AUDIT_OFERTARE.md` |
