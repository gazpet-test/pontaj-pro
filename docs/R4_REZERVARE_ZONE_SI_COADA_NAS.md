...
Teste finale pe ramură (pe capul ramurii, după runda 10; toate cu `--node-modules-dir=none --no-lock`, `deno.lock` neatins, md5 `875e293d…`):

| Comandă | Rezultat |
|---|---|
| `deno test supabase/functions/ofertare-plansa-citeste` | **147/147** (după `3197aa3`: 141/141; după `b5e7ecd`: 132/132; după `e8489a6`: 122/122; după `f1c4b66`: 109/109; după `f1274a1`: 100/100; după `a9fe186`: 89/89; după `a800d38`: 78/78) |
| `deno test -A supabase/functions/` | **164/164** (după `3197aa3`: 158/158; după `b5e7ecd`: 149/149; după `e8489a6`: 139/139; după `f1c4b66`: 126/126; după `f1274a1`: 117/117; după `a9fe186`: 106/106; după `a800d38`: 95/95) |
| `concurenta_test.ts` rulat de 10 ori | 10/10 verzi, 78/78 de fiecare dată |
...
### 5.7 Limite cunoscute (documentate, nu blochează) — actualizat în runda 10
...
**Ce a mai devenit vizibil în runda 10:** subtotalul cu materialul grupului („Total conducte PE De 110”) lângă poziția reală fără material — acum poziția primește cifra (sau, pe „doar de verificat”, se golește), iar subtotalul nota; TOTAL-ul cu interval „÷”, „/”, „la”, „până la” sau cu listă de Dn nu mai primește tăcut subtotalul primului Dn; „… lungime totală” lângă o altă poziție pe același Dn, cu un grup fără material, e din nou ambiguu vizibil (nu cifra pe cealaltă poziție).
...
- **La transfer:**
  ...
  - rândul TOTAL = „total” în denumire cu **niciun Dn standard, ≥ 2 Dn-uri sau un interval** (runda 9; runda 8 lua doar „fără Dn”, iar „Total rețea De 63–110” devenea candidat Dn63 și primea subtotalul în rândul TOTAL validat, cu poziția Dn63 dispărută); un „total” cu **un singur** Dn e candidat pe Dn-ul lui. Un **subtotal** (denumire care începe cu „total” / „subtotal”, după numerotare sau semne; runda 10) e candidat doar când e singurul de pe Dn: lângă o poziție fără „total” pe același Dn primește doar notă, **oricare ar fi materialul din denumirea lui** (runda 10: prioritatea se aplică înaintea filtrului pe material, în ținte și la „doar de verificat”). O poziție care conține „total” în altă parte („… lungime totală”) e poziție reală (runda 10). Rămân:
    ...
    - un subtotal care **nu** începe cu „total” („Lungime totală conducte De 110”, „Conducte De 110 — total”) e tratat ca poziție reală: lângă altă poziție pe același Dn, fără material care să departajeze, iese AMBIGUU (vizibil, `ambigue`), nu notă;
    - intervalul se recunoaște pe „-”, „–”, „—”, „÷”, „/”, „…” / „...”, „la”, „până la” între două numere cu prefix Dn/De/Ø (al doilea prefix opțional), iar lista pe „,”, „;”, „+”, „&”, „și”, „sau” (runda 10). Nu se recunosc: numere fără prefix („Total 63÷110” — dar fără Dn e oricum TOTAL), numere separate doar prin spațiu („Dn 63 90 110” ⇒ subtotal Dn63), un interval descrescător cu un capăt nestandard; un al doilea capăt / element urmat de „m”, „ml”, „km”, „%”, „buc”, „kg”, „ore” sau de zecimale nu e Dn (fals-negativ posibil: „Dn 63 și 110 m” ⇒ subtotal Dn63). „/” e și separatorul grosimii („De 110/10”): cu al doilea număr mai mic și nestandard nu e interval; „De 110/100” (improbabil) ar fi. Pe BD nu există nicio denumire „total” cu Dn, interval sau listă de Dn (SELECT 26.09: 19 denumiri „total”, toate încep cu „total”, 0 cu Dn);
