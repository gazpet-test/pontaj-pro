...
Stare: **doar propunere**. În BD s-au rulat numai SELECT-uri, iar documentul nu modifică nici codul, nici datele (codul e pe ramurile din §6.3 și §6.5). Data: 25.09.2026, revizia 6: 26.09.2026, ~07:00Z, **revizia 7: 26.09.2026, ~09:00Z** (capetele finale).

**Starea codului la care se referă documentul: 26.09.2026, ~09:00Z** (`git log` pe fiecare ramură, verificat la revizia 7). Textul de mai jos care citează `7a7bf86` / `6313184` descrie starea reviziei 6; unde revizia 7 schimbă starea, o spune explicit (marcat „Rev. 7”):

| Ramura | Capul | Ce conține | Verificare independentă | Pushată / PR / deployată |
|---|---|---|---|---|
| `claude/r4-rezervare-zone` | **`3039f2a`** (docs, 08:09:55Z); codul în **`1cc92bd`** (08:07:53Z), `COD_VERSIUNE` 2026-09-26.16 | deduplicarea pe identitatea rândului, rundele 3–12 (§6.5) | majorele TOTAL-a / TOTAL-b ale reverificării rundei 9 (și regresiile rundelor 10–11) închise în runda 12; **reverificarea adversarială a rundei 12 rămâne de făcut**; limite documentate în R4 §5.7 | nu / nu / nu |
| `claude/cantitati-nevalidate-consumatori` | **`a15a62a`** (docs, 08:48:46Z; cod `10e107d`), reașezată peste `3039f2a` (rebase-ul 4; conflictul din `treciInCantitati` rezolvat în `d167e94`: `candidatiPe` din R4 înlocuiește `candidatiDn`) | protecția consumatorilor + condițiile Copilot 1–2 + cele 6 decizii din audit, reversibile (R5 consumatori §11) + tot codul R4 | suitele trec: `-A supabase/functions/` 214/214 (planșa 192/192); **marcajul pasului B aliniat** (§6.4, rev. 7) | nu / nu / nu |
...
Revizii:
- **Revizia 7 (26.09, ~09:00Z):** capetele finale (R4 `3039f2a`, runda 12; consumatori `a15a62a`, rebase-ul 4 + deciziile din audit). Marcajul pasului B e aliniat prin decizia 1: codul recunoaște ambele forme, „ | R5 pas B” (cea scrisă de SQL-ul din §7.3) și „ | R5 v2 pas B”; valoarea verificată de om e `cantitate`, recitirea se compară cu ea și n-o scrie; `cantitate_plansa` se actualizează, dar nu pe un rând validat pe care recitirea îl confirmă; marcajul rămâne o singură dată la sfârșitul notei, deci RB îl găsește (testat pe ambele marcaje, `cantitati_nevalidate_test.ts`). SQL-ul pasului B din §7.3 nu se schimbă. Criteriile rămân 0 / 6 închise.
...
**Rev. 7:** capul e `a15a62a` (cod `10e107d`, peste R4 `3039f2a`); deciziile din audit sunt în `docs/R5_CONSUMATORI_CANTITATI_NEVALIDATE.md` §11. Rămân: migrările cu GO (întâi `R5_MIGRARE_PROPUSA_aprobare_istoric.sql`, apoi `R5_MIGRARE_PROPUSA_cantitati_nevalidate.sql`); `v_ofertare_pt_stare.qm` cere încă exact 'm' (semnalat prin `um_de_normalizat*`); lic. 5 are 15 rânduri F3 „M” (articole de deviz în „Conducte și montaj”) care după migrare apar în setul de rețea, cu H2 BLOCK — corecția e categoria lor; rândul niciodată validat scos din rețea prin categorie / „total” fără semnal dedicat (§11.7). Textul de mai jos e starea reviziei 6.
...
**Rev. 7 — marcajul pasului B, rezolvat (decizia 1 din audit, reversibilă; ramura consumatorilor `a15a62a`):** codul recunoaște ambele forme, „ | R5 pas B” (documentată, scrisă de SQL-ul din §7.3) și „ | R5 v2 pas B”; eticheta numește `cantitate` drept valoarea verificată de om; recitirea se compară cu ea și n-o scrie; `cantitate_plansa` se actualizează, cu excepția rândului validat pe care recitirea îl confirmă; marcajul rămâne o dată, la sfârșitul notei (RB îl găsește; testat pe ambele marcaje în `cantitati_nevalidate_test.ts`). Probele de mai jos, pe `6313184`, descriu starea dinainte; după orice retransfer, starea lui 1756 se recitește (B-verif) în continuare.
...
**Rev. 7:** capul e `3039f2a` (cod `1cc92bd`, `COD_VERSIUNE` 2026-09-26.16; rundele 10–12). Majorele TOTAL-a / TOTAL-b de mai jos sunt închise: capătul crescător fără prefix pe listă neagră de unități; „total” nerecunoscut ca subtotal lângă o poziție compatibilă ⇒ AMBIGUU, nimic scris; după „Cap./Art./Poz./Pct.” doar token de numerotare; pozițiile de alt material scoase din candidați. Teste: planșa 159/159, `-A supabase/functions/` 176/176, 52/52 mutații; fixture neschimbate (470 = 133 / 48.905 m, 130 = 18 / 37.320 m); BD (SELECT): 19 denumiri cu „total”, 0 cu Dn. Rămân: reverificarea adversarială a rundei 12 și limitele din R4 §5.7 (inclusiv costul fail-safe-ului: un subtotal real nerecunoscut blochează scrierea pe Dn-ul lui până la verificarea omului, vizibil). Textul de mai jos e starea reviziei 6.
...
## Jurnal de corecturi
### Revizia 7 (26.09, ~09:00Z): capetele finale
- R4 `3039f2a` (runda 12: TOTAL-a / TOTAL-b închise; reverificarea rundei 12 de făcut); consumatori `a15a62a` (rebase-ul 4, deciziile din audit). Marcajul pasului B: rezolvat prin decizia 1 (§6.4). Antetul, criteriile 1–2, §6.3–§6.5 marcate „Rev. 7”; restul textului păstrează starea reviziei 6, datată.
