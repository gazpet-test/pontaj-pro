...
// R5 sarcina 2: sursa citită, fără conflicte de transfer deschise, fără erori de citire
const sursa = { conflicte: [], eroare_conflicte: null, eroare_istoric: null }

describe('calculeazaPoartaGrafic', () => {
  it('toate intrările în platformă, fronturi din rânduri validate => nimic block', () => {
    const g = calculeazaPoartaGrafic({ p, cantitati: CANT, norme, cerinte, durataMax: durataMaxDinCerinte(cerinte), sursa })
  ...
  it('recalcul pe cantitățile RECITITE: un rând redeschis între încărcare și click => „cant" ȘI „front" block (înainte: doar „cant")', () => {
    ...
    const g = calculeazaPoartaGrafic({ p, cantitati: proaspete, norme, cerinte, durataMax: 12, sursa })
  ...
  it('durata peste maximul din cerințe => block (neschimbat)', () => {
    ...
    expect(stare(calculeazaPoartaGrafic({ p: { ...p, durata_luni: 14 }, cantitati: CANT, norme, cerinte, durataMax: 12, sursa }), 'durata')).toBe('block')
  })
  ...
  it('rând invalidat pe material după propunere => „cant” block cu `lista` (#2), „front” marcat incomplet; ce se îngheață conține lista', () => {
    ...
    const g = calculeazaPoartaGrafic({ p, cantitati: inval, norme, cerinte, durataMax: 12, sursa })
