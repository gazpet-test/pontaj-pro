...
// Returneaza File nou JPEG comprimat, sau fisierul original daca nu e nevoie
  ...
  return (
    <>
      <div
        ...
        <div style={{fontSize:11, color:G.muted}}>{hint || 'Imagini (JPG/PNG/WEBP, auto-resize la 2048px) sau PDF · max 15MB'}</div>
      </div>
...
function ScannerModal({ onClose, profile, showToast, onSaved }) {
  ...
  const sendToAI = async () => {
    ...
    try {
      // Resize automat daca e imagine mare (Anthropic Vision limit 5MB)
      let fileToSend = file
      if (file.type.startsWith('image/') && file.size > 2 * 1024 * 1024) {
        try {
          fileToSend = await resizeImageToBlob(file, 2048, 0.85)
          console.log(`[scan] Resized image: ${(file.size/1024/1024).toFixed(1)}MB -> ${(fileToSend.size/1024/1024).toFixed(1)}MB`)
        } catch (e) {
          console.warn('[scan] Resize fail, sending original:', e)
        }
      }
