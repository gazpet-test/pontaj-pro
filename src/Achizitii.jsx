...
function DocumenteComandaSection({ comanda, profile }) {
  ...
  // Upload un singur fișier (helper intern)
  const uploadOne = async (tip, blob, numeFisier, contentType) => {
    const ext = (numeFisier.split('.').pop() || 'pdf').toLowerCase()
    const path = `documente/${comanda.id}/${tip}_${Date.now()}_${uniq8()}.${ext}`
    const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, blob, { contentType: contentType || 'application/pdf' })
    if (upErr) throw upErr
    const { error: insErr } = await supabase.from('comenzi_furnizor_documente').insert({
      comanda_id: comanda.id, tip, fisier_path: path, fisier_nume: numeFisier, uploadat_de: profile?.id || null,
    })
    if (insErr) throw insErr
  }

  // Upload multiplu + ZIP auto-extract (12.06.2026): un .zip cu certificate se desface
  // singur și fiecare PDF/imagine din el urcă individual (mai bine pentru audit decât un PDF unit)
  const ZIP_EXT_OK = ['pdf', 'jpg', 'jpeg', 'png', 'webp']
  const uploadDocs = async (tip, fileList) => {
    ...
    try {
      for (const file of files) {
        if ((file.name || '').toLowerCase().endsWith('.zip')) {
          if (file.size > 60 * 1024 * 1024) { alert(`ZIP prea mare (max 60MB): ${file.name}`); skip++; continue }
          const zip = await JSZip.loadAsync(file)
          for (const [name, entry] of Object.entries(zip.files)) {
            if (entry.dir) continue
            const base = name.split('/').pop()
            // Skip junk macOS/Windows + fișiere ascunse
            if (!base || base.startsWith('.') || name.includes('__MACOSX')) continue
            const ext = (base.split('.').pop() || '').toLowerCase()
            if (!ZIP_EXT_OK.includes(ext)) { skip++; continue }
            const blob = await entry.async('blob')
            if (blob.size > 20 * 1024 * 1024) { skip++; continue }
            const ct = ext === 'pdf' ? 'application/pdf' : `image/${ext === 'jpg' ? 'jpeg' : ext}`
            await uploadOne(tip, blob, base, ct)
            ok++
          }
        } else {
          if (file.size > 20 * 1024 * 1024) { alert(`Fișier prea mare (max 20MB): ${file.name}`); skip++; continue }
          await uploadOne(tip, file, file.name, file.type)
          ok++
        }
      }
      await load()
      if (skip > 0) alert(`${ok} document(e) încărcate · ${skip} sărite (tip nesuportat sau prea mari)`)
  ...
  const UpBtn = ({ tip, label, color, multi }) => (
    <label style={{ ...S.btnS, fontSize: 13, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, color, borderColor: color + '66', fontWeight: 700, opacity: uploading ? .6 : 1 }}>
      ...
      <input type="file" accept={multi ? 'application/pdf,image/*,.zip' : 'application/pdf,image/*'} multiple={!!multi}
        disabled={!!uploading} onChange={e => { uploadDocs(tip, e.target.files); e.target.value = '' }} style={{ display: 'none' }} />
    </label>
  )
  const DocRow = ({ d }) => (
    ...
    <div style={{ marginTop: 14, padding: 14, background: G.surface, border: `1px solid ${G.border}`, borderRadius: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div>
          ...
          <UpBtn tip="calitate" label="🏅 Încarcă documente calitate" color={G.achizitii} multi />
          <div style={{ fontSize: 10.5, color: G.dim, marginTop: 6 }}>Selectare multiplă sau .zip (se desface automat). Vizibile în Execuție → Proiect → Documente.</div>
        </div>
      </div>
    </div>
  )
