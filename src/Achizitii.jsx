...
export default function AchizitiiPage() {
  ...
  const [cereriStats, setCereriStats] = useState({ deschis: 0, in_lucru: 0, comandata: 0, in_tranzit: 0, ajunsa: 0, finalizata: 0 })
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [rCom, rProj, rFz, rCtr, rSites, rProf, rEmp, rApr, rCer] = await Promise.all([
        ...
        // Cereri interne (comenzi de execuție) — pentru panoul sumar + badge
      ...
      const cs = { deschis: 0, in_lucru: 0, comandata: 0, in_tranzit: 0, ajunsa: 0, finalizata: 0 }
      for (const c of (rCer.data || [])) if (cs[c.status] !== undefined) cs[c.status]++
      setCereriStats(cs)
    } catch (e) {
      console.error(e)
  ...
  return (
    <div style={{ ...S.page, padding:'24px 0' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:14, borderBottom:`1px solid ${G.border}`, paddingBottom:0 }}>
        {[['comenzi', '🛒 Comenzi active'], ['arhiva', `📁 Arhivă${arhivaCount ? ` (${arhivaCount})` : ''}`], ['cereri_interne', `📋 Cereri interne${cereriStats.deschis ? ` 🔴 ${cereriStats.deschis}` : ''}`], ...(isOwner ? [['aprobatori', '👥 Aprobatori']] : [])].map(([k, t]) => (
      ...
      {tab !== 'aprobatori' && tab !== 'cereri_interne' && (<>
        ...
        {/* Panou sumar Cereri interne (comenzi de execuție) */}
        {tab === 'comenzi' && (
          <div style={{ ...S.card, padding:'14px 16px', marginBottom:14 }}>
            <div style={{ display:'flex', alignItems:'center', marginBottom:12 }}>
              <div style={{ fontSize:14, fontWeight:800, flex:1 }}>📋 Cereri interne</div>
              <button onClick={() => setTab('cereri_interne')} style={{ ...S.btnS, fontSize:12.5, color:G.achizitii, borderColor:G.achizitii + '55' }}>Vezi toate →</button>
            </div>
            <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
              {[
                ['🔴', 'Nepreluate', cereriStats.deschis, G.red],
                ['🔧', 'În lucru', cereriStats.in_lucru, G.yellow],
                ['🛒', 'Comandate', cereriStats.comandata, G.purple],
                ['🚚', 'În tranzit', cereriStats.in_tranzit, G.orange],
                ['📦', 'Ajunse', cereriStats.ajunsa, G.orange],
                ['✅', 'Finalizate', cereriStats.finalizata, G.green],
              ].map(([e, l, v, c], i) => (
                <div key={i} onClick={() => setTab('cereri_interne')}
                  style={{ flex:1, minWidth:120, background:G.bg, borderRadius:10, padding:'10px 12px', cursor:'pointer', border:`1px solid ${G.border}` }}>
                  <div style={{ fontSize:11.5, color:G.muted, marginBottom:3 }}>{e} {l}</div>
                  <div style={{ fontSize:20, fontWeight:800, color: v ? c : G.dim }}>{v}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {receptieBucatiComanda && (
        <ReceptieBucatiModal
          open={!!receptieBucatiComanda}
          proiectId={receptieBucatiComanda.proiect_id || null}
          comandaFurnizorId={receptieBucatiComanda.id}
          defaultProvenienta="gazpet"
          onClose={() => setReceptieBucatiId(null)}
          onSuccess={() => loadAll()}
        />
      )}
    </div>
